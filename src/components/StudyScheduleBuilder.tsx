...
// Demo ThemeContext for standalone use
...
// Mock GenerateScheduleModal for demo
const GenerateScheduleModal = ({ isOpen, onClose, onScheduleGenerated }) => {
  ...
  const handleGenerate = () => {
    ...
    const mockSchedule = [
      {
        topics: ['Logical Reasoning', 'Assumptions'],
        weekStart: '2025-08-14',
        section: 'Fundamentals',
        estimatedHours: 10,
        difficulty: 'Beginner'
      },
      {
        topics: ['Reading Comprehension', 'Main Point'],
      ...
      {
        topics: ['Logic Games', 'Sequencing'],
        weekStart: '2025-08-28',
        section: 'Practice',
        estimatedHours: 12,
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className={`rounded-2xl shadow-2xl max-w-lg w-full ${isDarkMode ? 'bg-slate-800' : 'bg-white'}`}>
        <div className="p-6">
          ...
          <p className={`mb-6 ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>
            Create a personalized LSAT study schedule based on your goals and timeline.
          </p>
...
const StudyScheduleBuilder = () => {
  ...
  return (
    <div className={`transition-colors duration-300 ${isDarkMode ? 'bg-slate-900' : 'bg-gray-50'}`} style={{ minHeight: 'calc(100vh - 64px)' }}>
...
// Demo ThemeProvider for standalone use
